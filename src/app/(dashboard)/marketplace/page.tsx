        ...
        <div className="p-5 pb-3 shrink-0">
          ...
          <div className="flex items-center gap-2 mb-5">
            <button
              ...
              className={cn(
                "px-5 py-2.5 text-[10px] font-black uppercase tracking-wider rounded-2xl border transition-all",
                activeTab === "browse"
              ...
              <div className="flex flex-col gap-3 mb-4">
                <div className="flex gap-2">
                  <div className="flex-1 flex items-center gap-2 rounded-xl border border-border bg-input px-3 py-2 transition-all focus-within:border-accent focus-within:ring-2 focus-within:ring-accent/10">
                    <MapPin size={16} weight="bold" className="text-muted shrink-0" />
                    <input
                ...
                <div className="flex items-center gap-2 rounded-xl border border-border bg-input px-3 py-2 transition-all focus-within:border-accent focus-within:ring-2 focus-within:ring-accent/10">
                  <MagnifyingGlass size={16} weight="bold" className="text-muted shrink-0" />
                  <input
          ...
          <div className="flex-1 overflow-y-auto px-5 pb-5 space-y-2.5">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 text-muted">
                ...
                <button onClick={fetchLoads} className="btn btn-secondary mt-4 text-[10px] font-black">
                  Retry
                </button>
              </div>
              ...
              <div className="flex flex-col items-center justify-center py-24 text-muted">
                ...
                <p className="font-bold uppercase tracking-widest text-xs">No loads found</p>
                <p className="text-[10px] text-muted mt-1">Try adjusting your filters</p>
              </div>
            ) : (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-2.5 items-center">
                      <div className="flex items-center gap-1 text-[10px] font-bold text-muted">
                        <CalendarBlank size={14} weight="bold" className="text-accent" />
                        {formatDate(load.pickupDate)}
                      </div>
                      <span className={cn("badge text-[9px] px-2 py-0.5", equipmentBadge(load.truckType))}>
                        {load.truckType}
                      </span>
                      <div className="flex items-center gap-1 text-[10px] font-bold text-muted">
                    ...
                    {load.broker?.riskScore !== undefined && (
                      <div className="mt-2 flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] font-bold text-muted">{load.broker.orgName}</span>
                      </div>
                    )}
                  </div>
                );
      ...
      <div className="w-[400px] overflow-y-auto bg-card shrink-0">
        {selectedLoad ? (
          <div className="p-5 space-y-6 animate-in fade-in duration-300">
            <div>
              <div className="text-[9px] font-black uppercase tracking-[0.2em] text-muted mb-1.5">Load Details</div>
            ...
            <div className="aspect-[16/10] w-full rounded-2xl bg-input border-2 border-dashed border-border flex items-center justify-center text-muted hover:border-accent/30 transition-colors cursor-pointer group">
              <div className="text-center group-hover:scale-105 transition-transform">
                ...
                <p className="text-[9px] font-black uppercase tracking-widest">Route Map</p>
                <p className="text-[10px] font-bold text-muted/60 mt-0.5">{selectedLoad.estimatedDistance || 0} miles</p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-y-5 gap-x-4">
              <div>
                <div className="text-[9px] font-black uppercase tracking-widest text-muted mb-1">Rate</div>
                <div className="text-xl font-black text-success tracking-tight">${selectedLoad.rate.toLocaleString()}</div>
                <div className="text-[9px] text-muted font-bold capitalize mt-0.5">{selectedLoad.rateType?.replace(/_/g, " ") || "flat rate"}</div>
              </div>
              <div>
                <div className="text-[9px] font-black uppercase tracking-widest text-muted mb-1">Equipment</div>
                <span className={cn("badge h-6 px-2.5 text-[10px]", equipmentBadge(selectedLoad.truckType))}>{selectedLoad.truckType}</span>
              </div>
              <div>
                <div className="text-[9px] font-black uppercase tracking-widest text-muted mb-1">Weight</div>
                <div className="flex items-center gap-1.5 text-[13px] font-black"><Scales size={16} weight="bold" className="text-muted" />{selectedLoad.weight?.toLocaleString() || "—"} lbs</div>
              </div>
              <div>
                <div className="text-[9px] font-black uppercase tracking-widest text-muted mb-1">Distance</div>
                <div className="flex items-center gap-1.5 text-[13px] font-black"><Truck size={16} weight="bold" className="text-muted" />{selectedLoad.estimatedDistance || 0} mi</div>
              </div>
              <div>
                <div className="text-[9px] font-black uppercase tracking-widest text-muted mb-1">Pickup</div>
                <div className="text-[12px] font-black text-foreground">{formatDate(selectedLoad.pickupDate, "full")}</div>
                <div className="text-[10px] font-bold text-muted">{formatTime(selectedLoad.pickupDate)}</div>
              </div>
              <div>
                <div className="text-[9px] font-black uppercase tracking-widest text-muted mb-1">Delivery</div>
                <div className="text-[12px] font-black text-foreground">{formatDate(selectedLoad.deliveryDate, "full")}</div>
                <div className="text-[10px] font-bold text-muted">{formatTime(selectedLoad.deliveryDate)}</div>
              </div>
            </div>

            <div className="rounded-2xl bg-input p-4 border border-border">
              <div className="text-[9px] font-black uppercase tracking-widest text-muted mb-3">Commodity</div>
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-accent text-[11px] font-black text-white shadow-lg shadow-accent/20"><Package size={18} weight="bold" /></div>
                <div className="text-[13px] font-black text-foreground">{selectedLoad.commodity || "General Freight"}</div>
              </div>
            </div>

            {selectedLoad.specialRequirements && (
              <div className="rounded-2xl bg-input p-4 border border-border">
                <div className="text-[9px] font-black uppercase tracking-widest text-muted mb-2">Requirements</div>
                <div className="flex flex-wrap gap-1.5">
                  {selectedLoad.specialRequirements.split(",").map((r) => (
                    <span key={r.trim()} className="badge badge-amber text-[9px] px-2 py-0.5">{r.trim()}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="flex h-full items-center justify-center text-muted">
            <div className="text-center p-8">
              ...
              <p className="font-bold uppercase tracking-widest text-xs">Select a load to view details</p>
            </div>
          </div>
        )}
      </div>
